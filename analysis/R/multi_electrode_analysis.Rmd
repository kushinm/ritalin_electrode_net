...
#base<- fread('data/rat6base.csv')
#drug<- fread('data/rat6drug.csv')
#base_acc<-fread('data/rat2accbase.csv')
#drug_acc<-fread('data/rat2accdrug.csv')
```

```{r}
calc_cv_acc<-function(full_x, full_y, a=0){
running_acc_dc ={}
base_pool = c(1:30)
drug_pool = c(31:60)
for (i in 0:9) {
...
base<-fread(paste0("data/rat",this_rat,"base.csv"))
drug<-fread(paste0("data/rat",this_rat,"drug.csv"))
full_x<- trimNA(rbind(base,drug))
full_y<-c(rep(0,30),rep(1,30))
this_data <- calc_cv_acc(full_x = full_x, full_y = full_y)
all_results<-append(all_results, this_data)
}
...
```{r}
results_list<- (all_results[seq(2,16,2)])
l={}
for (this_rat in 1:num_rats){

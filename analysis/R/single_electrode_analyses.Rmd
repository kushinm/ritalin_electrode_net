...
for (i in 0:9) {
  ...
  model_dg <- cv.glmnet(data_cols,fit_dg,family = "binomial") ## Fit model for drug condition
  ...
  dg_acc= 1-(sum(abs(as.numeric(test_dg)-pred_dg))/length(pred_dg)) 
...
for (i in 0:9) {
  ...
  model_dg <- cv.glmnet(data_cols,fit_dg,family = "binomial")
